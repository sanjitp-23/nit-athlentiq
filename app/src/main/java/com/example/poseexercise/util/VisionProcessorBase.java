 ...
 * {@link
 * #onSuccess(Object, GraphicOverlay)} to define what they want to with the
...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
    ...
    // Frame count that have been processed so far in an one second interval to
    // calculate FPS.
    ...
    // Latest bitmap from camera frame (for multi-person detection)
    @Nullable
    ...
    // -----------------Code for processing live preview frame from CameraX
    // API-----------------------
    @Override
    ...
    public void processImageProxy(ImageProxy image, GraphicOverlay graphicOverlay) {
        ...
        // Always create a bitmap for person detection pipeline
        Bitmap bitmap = BitmapUtils.getBitmap(image);
        latestBitmap = bitmap;

        if (PreferenceUtils.isCameraLiveViewportEnabled(graphicOverlay.getContext())) {
            bitmap = null; // Don't use for camera image graphic when viewport is on
        }

        if (isMlImageEnabled(graphicOverlay.getContext())) {
            MlImage mlImage = new MediaMlImageBuilder(image.getImage())
            ...
            requestDetectInImage(
                    ...
                    // received
                    // images when finished using them. Otherwise, new images may not be received or
                    // the
                    // camera may stall.
                    // Currently MlImage doesn't support ImageProxy directly, so we still need to
                    // call
    ...
    private Task<T> setUpListener(
        ...
        return task.addOnSuccessListener(
                ...
                results -> {
                    ...
                    // Only log inference info once per second. When
                    // frameProcessedInOneSecondInterval is
                    // equal to 1, it means this is the first frame processed during the current
                    // second.
