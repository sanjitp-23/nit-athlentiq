...
internal constructor(
    ...
    private val offsetX: Float = 0f,  // Offset for cropped image origin
    private val offsetY: Float = 0f   // Offset for cropped image origin
) : Graphic(overlay) {
    ...
    /** Translate X coordinate with crop offset applied */
    private fun tx(x: Float): Float = translateX(x + offsetX)

    /** Translate Y coordinate with crop offset applied */
    private fun ty(y: Float): Float = translateY(y + offsetY)
        ...
        if (showInFrameLikelihood) {
            for (landmark in landmarks) {
                canvas.drawText(
                    ...
                    tx(landmark.position.x),
                    ty(landmark.position.y),
                    whitePaint
                )
            }
        }
    ...
    private fun drawPoint(canvas: Canvas, landmark: PoseLandmark, paint: Paint) {
        ...
        canvas.drawCircle(tx(point.x), ty(point.y), DOT_RADIUS, paint)
    }
