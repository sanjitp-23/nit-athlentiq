...
public class GraphicOverlay extends View {
    ...
    // --- Person selection support ---
    public interface OnPersonTapListener {
        void onPersonTapped(int trackingId);
    }

    private OnPersonTapListener personTapListener;
    private List<TrackedPerson> currentPersons = new ArrayList<>();

    public void setOnPersonTapListener(OnPersonTapListener listener) {
        this.personTapListener = listener;
    }

    public void setCurrentPersons(List<TrackedPerson> persons) {
        this.currentPersons = persons != null ? persons : new ArrayList<>();
    }
    // --- End person selection support ---
    ...
    public GraphicOverlay(Context context, AttributeSet attrs) {
        ...
        addOnLayoutChangeListener(
                (view, left, top, right, bottom, oldLeft, oldTop, oldRight,
                        oldBottom) -> needUpdateTransformation = true);
    }
    ...
    /**
     * Handle touch events for person selection.
     * Converts view coordinates back to image coordinates and checks
     * if the tap falls within any detected person's bounding box.
     */
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (personTapListener == null || currentPersons.isEmpty()) {
            return super.onTouchEvent(event);
        }
        ...
        // Check each person's bounding box (transformed to view coordinates)
        for (TrackedPerson person : currentPersons) {
            Rect box = person.getBoundingBox();
            float left, right;
            if (isImageFlipped) {
                left = getWidth() - (box.right * scaleFactor - postScaleWidthOffset);
                right = getWidth() - (box.left * scaleFactor - postScaleWidthOffset);
            } else {
                left = box.left * scaleFactor - postScaleWidthOffset;
                right = box.right * scaleFactor - postScaleWidthOffset;
            }
            float top = box.top * scaleFactor - postScaleHeightOffset;
            float bottom = box.bottom * scaleFactor - postScaleHeightOffset;

            if (tapX >= left && tapX <= right && tapY >= top && tapY <= bottom) {
                personTapListener.onPersonTapped(person.getTrackingId());
                return true;
            }
        }
